...
# Паспортное «Введение» источника. Книжная мукаддима тома — рекламная (почему этот
# перевод «первый богословский» и т. п.), поэтому в паспорт НЕ идёт: даём короткое
# нейтральное описание. Редакторские замечания к содержанию — в блоке предупреждений.
ABOUT_TEXT = (
    "«Коран. Богословский перевод» — смысловой перевод Корана на русский язык с "
    "комментариями. Автор — Шамиль Аляутдинов (современный, ханафитского направления).\n\n"
...
def main():
    ...
    # about / «Введение»: фиксированное нейтральное описание (не книжная мукаддима).
    _ = book_intro  # мукаддима больше не используется (рекламная); оставлено для ясности
    os.makedirs(os.path.dirname(OUT_ABOUT), exist_ok=True)
    open(OUT_ABOUT, "w", encoding="utf-8").write(ABOUT_TEXT + "\n")
