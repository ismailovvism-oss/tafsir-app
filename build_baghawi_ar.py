#!/usr/bin/env python3
"""
Арабский оригинал тафсира аль-Багави «Ма'алим ат-Танзиль» (baghawi_ar).

Источник: tafsir_api (spa5k), издание ar-tafsir-al-baghawi, по-аятно на jsdelivr CDN:
  https://cdn.jsdelivr.net/gh/spa5k/tafsir_api@main/tafsir/ar-tafsir-al-baghawi/<sura>/<ayah>.json
  → {"text": "...", "ayah": n, "surah": s}. Текст чистый (без HTML), абзацы \\n\\n,
  ﴿…﴾ — цитаты аятов, [[…]] — редакторские примечания → переводим в сноски [^N].

Кеш: data/_sources/baghawi_ar/<sura>/<ayah>.json (gitignored, резюмируемо).
Выход: data/tafsirs/baghawi_ar.json = {"<sura>": {"<ayah>": "md"}}.

Использование:
  python3 build_baghawi_ar.py --sample 67:1 2:255
  python3 build_baghawi_ar.py                 # fetch + parse + монолит
Дальше: split.py baghawi_ar && build_index.py baghawi_ar && compute_fill.py && build_coverage.py
"""
...
CACHE = os.path.join(ROOT, "data", "_sources", "baghawi_ar")
OUT = os.path.join(ROOT, "data", "tafsirs", "baghawi_ar.json")
BASE = "https://cdn.jsdelivr.net/gh/spa5k/tafsir_api@main/tafsir/ar-tafsir-al-baghawi/{s}/{a}.json"
...
def main():
    ...
    if not a or "--parse" in a:
        ...
        print("\nДальше: python3 split.py baghawi_ar && python3 build_index.py baghawi_ar "
